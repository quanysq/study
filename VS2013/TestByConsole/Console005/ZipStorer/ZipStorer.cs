...
namespace Console005.ZipStorer
{
  class ZipStorer : IDisposable
  {
    ...
    static ZipStorer()
    {
      ...
      CustomLogger.DEFAULT.InfoFormat("CrcTable count: [{0}]", CrcTable.Length);
      CustomLogger.DEFAULT.InfoFormat("CrcTable content: [{0}]", string.Join(", ", CrcTable));
    }
    ...
    public static ZipStorer Open(string _filename, FileAccess _access)
    {
      Stream stream =
        (Stream)
        new FileStream(_filename, FileMode.Open, _access == FileAccess.Read ? FileAccess.Read : FileAccess.ReadWrite);
    ...
    private void Store(ref ZipFileEntry _zfe, Stream _source)
    {
      ...
      CustomLogger.DEFAULT.InfoFormat("Zip's Position is: [{0}]", posStart);
      ...
      CustomLogger.DEFAULT.InfoFormat("[1] [{0}]'s CRC32 is: [{1}]", _zfe.FilenameInZip, _zfe.Crc32);
    ...
    private void UpdateCrcAndSizes(ref ZipFileEntry _zfe)
    {
      ...
      CustomLogger.DEFAULT.InfoFormat("[2] [{0}]'s CRC32 is: [{1}]", _zfe.FilenameInZip, _zfe.Crc32);
      var crcBytes = BitConverter.GetBytes(_zfe.Crc32);
      CustomLogger.DEFAULT.InfoFormat("[{0}]'s CRC32 Bytes size: [{1}]", _zfe.FilenameInZip, crcBytes.Length);
